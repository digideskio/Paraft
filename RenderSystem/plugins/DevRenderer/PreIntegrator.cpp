...
void PreIntegrator::generateTable1(float *table, float *tf, int resolution)
{
    ...
    if (i <= j)
    {
        float factor = _sampleStep / ((float)(j - i + 1) * _baseSample);
        for (int k = i; k <= j; k++)
        {
            float alpha = 1.0f - pow(1.0f - tf[k * 4 + 3], factor);
            float accAlpha = (1.0f - color.w) * alpha;
            color.x += accAlpha * tf[k * 4];
            color.y += accAlpha * tf[k * 4 + 1];
            color.z += accAlpha * tf[k * 4 + 2];
            color.w += accAlpha;
        }
    }
    else
    {
        float factor = _sampleStep / ((float)(i - j + 1) * _baseSample);
        for (int k = i; k >= j; k--)
        {
            //color.x += (1.0f - color.w) * tf[k * 4];
            ///color.y += (1.0f - color.w) * tf[k * 4 + 1];
            //color.z += (1.0f - color.w) * tf[k * 4 + 2];
            //float alpha = 1.0f - pow(1.0f - tf[k * 4 + 3], factor);
            //color.w += (1.0f - color.w) * alpha;
            float alpha = 1.0f - pow(1.0f - tf[k * 4 + 3], factor);
            float accAlpha = (1.0f - color.w) * alpha;
            color.x += accAlpha * tf[k * 4];
            color.y += accAlpha * tf[k * 4 + 1];
...
void PreIntegrator::generateTable3(float *table, float *tf, int resolution)
{
    ...
    for (int dist = 1; dist < resolution; dist++)
    {
        ...
        int index = (dist * resolution + 0) * 4;
        color.set(&table[index]);
        table[index + 3] = alpha;
        ...
        for (int i = 0; i < resolution - dist - 1; i++)
        {
            int j = i + dist;
            //float ri = (tf[i * 4]     + tf[(i + 1) * 4]    ) * 0.5f;
            //float gi = (tf[i * 4 + 1] + tf[(i + 1) * 4 + 1]) * 0.5f;
            //float bi = (tf[i * 4 + 2] + tf[(i + 1) * 4 + 2]) * 0.5f;
            //float ai = 1.0f - pow(1.0f - (tf[i * 4 + 3] + tf[(i + 1) * 4 + 3]) * 0.5f, factor);
            //float rj = (tf[j * 4]     + tf[(j - 1) * 4]    ) * 0.5f;
            //float gj = (tf[j * 4 + 1] + tf[(j - 1) * 4 + 1]) * 0.5f;
            //float bj = (tf[j * 4 + 2] + tf[(j - 1) * 4 + 2]) * 0.5f;
            //float aj = 1.0f - pow(1.0f - (tf[j * 4 + 3] + tf[(j - 1) * 4 + 3]) * 0.5f, factor);
            ...
            int index = ((j + 1) * resolution + i + 1) * 4;
            color.set(&table[index]);
            table[index + 3] = alpha;
        ...
        color = Vector3f();
        alpha = 0.0f;
        for (int m = resolution - 1, n = resolution - 2; m > resolution - 1 - dist; m--, n--)
        {
            //Vector4f c((tf[m * 4] + tf[n * 4]) * 0.5f,
            //           (tf[m * 4 + 1] + tf[n * 4 + 1]) * 0.5f,
            //           (tf[m * 4 + 2] + tf[n * 4 + 2]) * 0.5f,
        ...
        ////
        count++;
        dif = abs(table[index] - table2[index]);
        if (max3 < dif) max3 = dif;

        //for (int i = 0; i < resolution - dist - 1; i++)
        for (int i = resolution - 1; i > dist; i--)
        {
            int j = i - dist;
            Vector3f ci = Vector3f(&tf[i * 4]);
            float ai = _adjAlpha(tf[i * 4 + 3], factor);
        ...
        */
        ...
        for (int sign = 1; sign >= -1; sign -= 2)   // sign = 1, -1
        {
            //color = Vector3f();
            //alpha = 0.0f;
            /*for (int m = (sign == 1 ? 0 : resolution - 1), n = 0; n < dist; m += sign, n++)
            {
                Vector3f c = Vector3f(&tf[m * 4]);
                float a = _adjAlpha(tf[m * 4 + 3], factor) * (1.0f - alpha);
                color += c * a;
