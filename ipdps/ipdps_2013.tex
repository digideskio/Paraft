...
Note that each voxel in the local data block has a unique global index, each leaf could thus be encoded using 3 integers (global index for $P_{centroid}$, $P_{min}$ and $P_{max}$). On the other hand, the root and the first level child nodes are encoded using the index of corresponding data blocks, irrelevant to the number of features-on-boundary (henceforth referred as $N_{fb}$). Therefore the spatial complexity of creating local connectivity tree for each data block is ($O(N_{fb})$). \textcolor{red}{Consider an extreme case that there are 1000 features touching each side of the boundary surface, the total memory needed for the local connectivity tree is only $((3*1000 + 1)*6 + 1) * 4 bytes \approx 72KB$, neglectable comparing to the volume size itself.}

From the temporal complexity perspective of view, the creation of local connectivity tree will not introduce much extra computational cost as it can be done along with the region growing process. The value of $P_{centroid}$ and $P_{min-max}$ are updated only if the feature reaches the boundary surface, hence the temporal complexity for creating the local connectivity tree remains ${O(\sqrt[2/3]{N_{vexol}})}$, a magnitude less than \textcolor{red}{$O(region-growing)$.}

Algorithm~\ref{alg:local} shows the detailed algorithm of creating local connectivity tree in the region growing process.
%------------------------------------------------
\begin{algorithm}
\caption{Creating Partial Connectivity Tree}
\label{alg:local}
\begin{algorithmic}
...
\ELSE
	\FOR {each $feture$ in $featureList$}
		...
		\STATE $feture \leftarrow \textbf{adjustRegion()}$
...
Local connectivity information need to be merged to obtain the overall description of a partitioned feature. A naive solution to gathering local connectivity information is to let each data block exchanging their leaves with its neighboring blocks. If a match was found for two corresponding boundary surfaces, the two leaves are merged.  at a processor if they satisfy the following three conditions:
