...
\caption{Match of two partial features}
	\begin{algorithmic}[1]
		\IF{$P_{centroid}$ = $P_{center}^{'}$ \textbf{and} $P_{min-max}$ = $P_{min-max}^{'}$}
...
\begin{figure}[ht]
	...
	\caption{The tree structure used for maintaining the local conneticity information. }
	\label{fig:match}
\end{figure}

Note that each voxel in the local data block has a unique global index, each leaf could thus be encoded using 3 integers (global index for $P_{centroid}$, $P_{min}$ and $P_{max}$). On the other hand, the root and the first level child nodes are encoded with the index of corresponding data blocks, irrelevant to the number of features-on-boundary (henceforth referred as $N_{fb}$). Therefore the spatial complexity of creating local connectivity tree for each data block is ($O(N_{fb})$). \textcolor{red}{Consider an extreme case that there are 1000 features touching each side of the boundary surface, the total memory needed for the local connectivity tree is only $((3*1000 + 1)*6 + 1) * 4 bytes \approx 72KB$, neglectable comparing to the volume size itself.}
...
A naive solution for obtaining global connectivity information for each feature-on-boundary is to send the leaves in the local connectivity trees together with parent node \textcolor{red}{so what?}. Recall that the first level child node of the root are encoded as the ranks of adjacent blocks, and the leaves the global index of the geometric centroid and the min-max boundary on the shared boundary surface. Therefore if a leaf received from neighboring data block matches a local leaf, the two leaves, that is, the two partial features must share the same boundary section with the same centroid and section region. In other word, these two partial features are resulted by partitioning a original feature, and should be considered the same feature.
...
\label{alg:merge}
\begin{algorithmic}[1]
\STATE sort $localLeaves$, $recievedLeaves$ by $P_{centroid}$
\FOR {each $leaf_{local}$ in $localLeaves$}
	\FOR {each $leaf_{recved}$ in $recievedLeaves$}
		\IF{$li \approx lj$}
			\IF {$ei.id < ej.id$}
